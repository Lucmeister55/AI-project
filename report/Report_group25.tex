...
For the architecture for the final layers which we added to the pretrained model we used the same layers as the previous model. \\ 
The training curve of the pretrained model has a much lower accuracy than the final baseline model. The loss curve is similar as in it goes down but it does not reaches as low as the baseline loss curve. \\
